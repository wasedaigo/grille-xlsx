...
var GrilleXlsx = function(file, metaTableName = 'meta', readAsData=false) {
  this.readAsData = readAsData;
  this.file = file;
...
GrilleXlsx.prototype.toJson = function() {
  if (this.readAsData) {
    return GrilleXlsx.xlsxDataToJson(this.file, this.metaTableName);
  } else {
    return GrilleXlsx.xlsxFileToJson(this.file, this.metaTableName);
  }
};
...
GrilleXlsx.processArray = function(result, sheetName, header, json, path) {
  ...
  GrilleXlsx.dotConcat(result, conent, path);
};
...
/**
 * Concats data in a deep object using dot notation
 */
GrilleXlsx.dotConcat = function(destination, source, path) {
  var nodes = path.split("."); // e.g. x, y, z
  ...
  var orig = pointer[nodes[nodes.length - 1]];
  if (orig && orig.length > 0) {
    pointer[nodes[nodes.length - 1]] = _.concat(orig, source);
  } else {
    pointer[nodes[nodes.length - 1]] = source;
  }
...
GrilleXlsx.xlsxFileToJson = function(filename, meta) {
  var workbook = xlsx.readFile(filename);
  return GrilleXlsx.processWorksheet(workbook, meta);
};

GrilleXlsx.xlsxDataToJson = function(data, meta) {
